...
$extract_product=mysqli_query($con,"SELECT * FROM `purchase` WHERE `transaction_id`='$transaction_id' AND `user_id`='$user_id'");
$row_product=mysqli_fetch_array($extract_product);
if(mysqli_num_rows($extract_product)>0)
{
	if($row_product['fullfilled']>=0)
		$com=$row_product['commision'];
	else
...
mysqli_query($con,"UPDATE `purchase` SET `fullfilled`='-1' WHERE `transaction_id`='$transaction_id' AND `user_id`='$user_id'");
}
?>
</form>
</body>
